...
local score=torch.Tensor(size,4):zero()
...
--print(gen[labelsShuffle[1]])
--local d=torch.Tensor(4096)
--for t=1,4096 do
 --  d[t]=trainData.data[1][t+4096]--:sub(4097,8192)
  -- if d[t]~=gen[1][t] then 
  --  print(d[t],gen[1][t])
 -- end
...
--print(d)
--for t=1,size do
--  local prob1=model_test:forward(gen[1])
 -- print(prob1)
  --print('pass 2')
  --local prob =model_test:forward(d)
  --maxval,maxid = prob:max(1)
  --print(prob)
 -- print(maxid[1])
--end

--local prob1=model:forward( trainData.data [1])
 --print(prob1)
 --local prob1=model:forward(testData.data[1])
 --print(prob1)
