...
local indata = {
data = torch.Tensor(size, 4096),
labels = torch.Tensor(size),
size = function() return size end
}
for i=1,size do

indata.labels[i] = labelsAll[i]
end
      ...
      iw = img:size(3)
       ih = img:size(2)
          ...
          --  print('==> Pass Through model------1')
            ...
            for j=1,10 do
              out_sc[j]=model2:forward(img_sc[j])
            end
            out_avg[i]=torch.sum(out_sc,1)
          end
      ...
      iw = img:size(3)
       ih = img:size(2)
          ...
          --  print('==> Pass Through model------1')
            ...
            for j=1,10 do
              out_sc[j]=model3:forward(img_sc[j])
            end
            out_avg[i]=torch.sum(out_sc,1)
          end
      ...
      iw = img:size(3)
       ih = img:size(2)
          ...
          --  print('==> Pass Through model------1')
            ...
            for j=1,10 do
              out_sc[j]=model4:forward(img_sc[j])
            end
            out_avg[i]=torch.sum(out_sc,1)
          end
      ...
      iw = img:size(3)
       ih = img:size(2)
          ...
          --  print('==> Pass Through model------1')
            ...
            for j=1,10 do
              out_sc[j]=model5:forward(img_sc[j])
            end
            out_avg[i]=torch.sum(out_sc,1)
          end
...
indata.data=torch.cat(indata.data,mustache,2)
--print('IN_GEN DATA')
--print(gen[1][1],gen[2][1])
----------------------------------------test model----------------------------------------------
...
local labelsShuffle = torch.randperm((#indata.labels)[1])
--print(labelsShuffle)


print '==> Create Train set'
trainData = {
data = torch.Tensor(trsize,opt.states,1),
...
--create test set:
 testData = {
data = torch.Tensor(tesize,opt.states,1),
labels = torch.Tensor(tesize),
size = function() return tesize end
}

for i=1,trsize do
trainData.data[i] = indata.data[labelsShuffle[i]]:clone()
trainData.labels[i] = indata.labels[labelsShuffle[i]]
--print(labelsShuffle[i])
end

for i=trsize+1,tesize+trsize do
testData.data[i-trsize] = indata.data[labelsShuffle[i]]:clone()
testData.labels[i-trsize] =  indata.labels[labelsShuffle[i]]
